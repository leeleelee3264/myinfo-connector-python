from .env import ENV

SAMPLE_PRIVATE_KEY = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"""

SAMPLE_MYINFO_PUBLIC_KEY = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""

MYINFO_HOST = ENV.str('MYINFO_HOST', 'https://test.api.myinfo.gov.sg/com/v3')

KASA_REDIRECT_URL = ENV.str('KASA_REDIRECT_URL', 'http://localhost:3001/callback')

MYINFO_CLIENT_ID = ENV.str('MYINFO_CLIENT_ID', 'STG2-MYINFO-SELF-TEST')
MYINFO_CLIENT_SECRET = ENV.str('MYINFO_CLIENT_SECRET', '44d953c796cccebcec9bdc826852857ab412fbe2')

MYINFO_GENERATED_PUBLIC_KEY = ENV.str('MYINFO_GENERATED_PUBLIC_KEY', SAMPLE_MYINFO_PUBLIC_KEY)
KASA_GENERATED_PRIVATE_KEY = ENV.str('KASA_GENERATED_PRIVATE_KEY', SAMPLE_PRIVATE_KEY)

MYINFO_REQUESTED_ATTRIBUTES = "name,dob,birthcountry,nationality,uinfin,sex,regadd,noa-basic"
